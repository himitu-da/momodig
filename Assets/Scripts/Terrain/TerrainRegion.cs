...
public class TerrainRegion : MonoBehaviour
{
    ...
    public void Initialize(TerrainManager manager)
    {
        ...
        {
            Debug.Log($"TerrainRegion: Initialized with TerrainManager");
        }
    }
    ...
    public ChunkData CreateChunk(Vector3Int chunkPos, Vector3 worldPos, bool[,,] pattern, TerrainSettings settings)
    {
        if (showChunkDebugInfo)
        {
            Debug.Log($"TerrainRegion: Creating chunk at {chunkPos}");
        }
        ...
        if (showChunkDebugInfo)
        {
            Debug.Log($"TerrainRegion: Created chunk with {chunkData.blockCount} blocks");
        }
    ...
    public void SetChunkActive(Vector3Int position, bool active)
    {
        ...
        if (chunk != null && chunk.block != null)
        {
            ...
            if (showChunkDebugInfo)
            {
                Debug.Log($"TerrainRegion: Set chunk {position} active: {active}");
            }
        }
    }
    ...
    public void ClearAllChunks()
    {
        if (showChunkDebugInfo)
        {
            Debug.Log($"TerrainRegion: Clearing {chunks.Count} chunks");
        }
    ...
    public string GetDebugInfo()
    {
        ...
        return $"TerrainRegion - Total: {chunks.Count}, Active: {GetActiveChunkCount()}, Blocks: {totalBlocks}";
    }
