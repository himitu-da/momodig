...
## 実装方針について
- よく調べて、計画を立ててから実装する
- 実装計画の承認があって初めて実装を開始する
- コードは変更されるものであると考えて、後から変更しやすいように実装する
- まずはシンプルに実装して、必要に応じてコードを育ててリファクタリングしていく

## ドキュメントについて
このプロジェクトには、このAGENTS.mdの他にもドキュメントが存在するときがあります。
- docs/ ディレクトリ以下に、設計方針やルールをまとめていることがありますので、必要に応じて参照してください
- ドキュメントは常に最新の状態になるように、実装やルールの変更に合わせて更新してください

## 基本方針
- このプロジェクトでは、Unity上に表示されるInspector・Hierarchy・Prefabの状態やSceneでの配置を正とする
  - HierarchyやInspectorで数値や範囲を調整しやすいことが重要です
  - runtimeでUIを自動生成して配置することは、基本的に避けるべきです
