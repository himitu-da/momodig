...
- 実行中: 30 秒ごとに autosave する
- アプリ中断時: `OnApplicationPause(true)` で保存する
- アプリ終了時: `OnApplicationQuit` で保存する
- 管理対象 Scene 遷移時: `GameSceneCoordinator` が遷移開始時に保存する

保存対象が完全に空の場合は、空の保存ファイルを新規作成せず、既存の保存ファイルを削除状態に保つ。
これにより、タイトルで簡易リセットした直後に autosave が空ファイルを再生成しない。
